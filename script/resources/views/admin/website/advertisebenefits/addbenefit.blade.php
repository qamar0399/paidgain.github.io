...
@section('title', __('Add New Advertise Benefit'))

@section('head')
    @include('layouts.backend.partials.headersection', ['title'=> __('Add New Advertise Benefit'),'prev' => url()->previous()])
@endsection
                    ...
                    <div class="tab-content no-padding" id="myTab2Content">
                        ...
                        @foreach($languages->value as $key => $value)
                            <div class="tab-pane fade {{ $i == 0 ? 'show active' : null }}" id="{{ $key }}" role="tabpanel" aria-labelledby="{{ $key }}-tab">
                                <form class="ajaxform_with_reset" action="{{ route('admin.website.heading.store-advertise-benefit') }}" method="POST">
                                    @csrf
                                    ...
                                    <div class="custom-control custom-switch">
                                        ...
                                        <label class="custom-control-label" for="featured">{{ __('Featured') }} ({{ $key }})</label>
                                    </div>
                                    <div class="form-group">
